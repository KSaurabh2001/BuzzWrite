...
import { getAllBlogsByUserId } from "../../Redux/Post/Action";
...
const AuthorProfile = () => {
  ...
  const { userById } = useSelector((store) => store.user);
  const { allPostsByUserId } = useSelector((store) => store.post);

  useEffect(() => {
    if (id && token) {
      dispatch(getUserById(id, token));
      dispatch(getAllBlogsByUserId(id, token));
    }
  }, [id, token, dispatch]);

  const handleFilterChange = (e) => setFilter(e.target.value);

  const filteredPosts = allPostsByUserId?.filter((post) =>
    filter === "title"
      ...
      <div className="relative z-10 px-6 py-10">
        ...
        <div className="max-w-4xl mx-auto bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 md:flex md:items-center mb-10">
          ...
          <div className="mt-6 md:mt-0 md:ml-8 text-center md:text-left">
            ...
            <p className="text-gray-600 dark:text-gray-300">@{userById?.username}</p>
            <p className="text-gray-700 dark:text-gray-400 mt-2">{userById?.occupation}</p>
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">{userById?.email}</p>
            <p className="text-sm text-gray-600 dark:text-gray-400">
              {userById?.gender} | DOB: {userById?.dob}
            </p>
            <p className="text-sm text-gray-600 dark:text-gray-400">{userById?.address}</p>
            <p className="mt-3 text-gray-800 dark:text-gray-200 italic">{userById?.bio}</p>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredPosts?.length > 0 ? (
            ...
            <p className="text-center col-span-full text-gray-500">
              {allPostsByUserId?.length === 0 ? "No posts found" : "Loading posts..."}
            </p>
          )}
        </div>
      </div>
